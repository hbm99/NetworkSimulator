...
class Send(Instruction):
    ...
    def execute(self, args):
        ...
        for i in range(len(data)):
            ...
            while time.time() * 1000 - initial_time <= simulator.signal_time:
                queue_bfs = [computer.port]
                while len(queue_bfs) > 0:
                    current_port : Port = queue_bfs.pop(0)
                    if current_port.cable == None:
                        continue
                    current_port.device.write(int(time.time() - simulator.start), computer.port, "send", data[i], "ok")
                    ...
                    destination_port.device.write(int(time.time() - simulator.start), destination_port, "receive", data[i], "ok")
